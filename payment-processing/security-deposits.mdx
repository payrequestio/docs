...
- **Hold Duration**: Configurable from 1 to 28 days (the bank's window is the hard ceiling — see [hold durations per PSP](/payment-processing/deposit-hold-duration))
...
<AccordionGroup>
  ...
  <Accordion title="What's the maximum hold duration?">
    You can configure hold durations from 1 to 28 days on the product. The actual maximum the bank enforces depends on the PSP and card brand — anywhere from 7 days (Visa via Stripe blended pricing) to 180 days (Vipps via Mollie). See [Deposit Hold Duration](/payment-processing/deposit-hold-duration) for the full per-PSP and per-network breakdown.
  </Accordion>
